...
function calculateSalaryFrom(fullSalary) {
  ...
  const percentageINSS = fullSalary === 0 ? 0 : discountINSS / fullSalary;

  const baseIRPF = baseINSS - discountINSS;
  const discountIRPF = calculateDiscountIRPF(baseIRPF);
  const percentageIRPF = fullSalary === 0 ? 0 : discountIRPF / fullSalary;

  const netSalary = baseINSS - discountINSS - discountIRPF;
  const percentageNetSalary = fullSalary === 0 ? 0 : netSalary / fullSalary;
